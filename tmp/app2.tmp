#!/usr/bin/env python3
"""
Quadratic Equation Solver v2.0
...
with col1:
    ...
    if input_type == "Decimal":
        a_str = st.text_input("", value="1", key="a", label_visibility="collapsed")
        a = parse_coefficient(a_str, False)
    else:
        a_str = st.text_input("", value="1", key="a_frac", label_visibility="collapsed")
        a = parse_coefficient(a_str, True)

with col2:
    st.markdown("**b** (x)")
    if input_type == "Decimal":
        b_str = st.text_input("", value="-3", key="b", label_visibility="collapsed")
        b = parse_coefficient(b_str, False)
    else:
        b_str = st.text_input("", value="-5/2", key="b_frac", label_visibility="collapsed")
        b = parse_coefficient(b_str, True)

with col3:
    st.markdown("**c**")
    if input_type == "Decimal":
        c_str = st.text_input("", value="2", key="c", label_visibility="collapsed")
        c = parse_coefficient(c_str, False)
    else:
        c_str = st.text_input("", value="3/4", key="c_frac", label_visibility="collapsed")
...
if a is not None and b is not None and c is not None:
    ...
    if st.button("Solve", type="primary", use_container_width=True):
        ...
        if a_float == 0:
            ...
            tab1, tab2 = st.tabs(["Solutions", "Graph"])
            ...
            with tab2:
                ...
                # Only show vertex info for real functions
